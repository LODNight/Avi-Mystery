    ...
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border bg-card p-3 shadow-sm">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-xl bg-primary px-3.5 py-2 text-xs font-bold text-primary-foreground hover:bg-primary/90 transition-all shadow-xs disabled:opacity-50 cursor-pointer"
          title="Chạy thử công thức trên ô tính hiện tại"
        >
          <Play className={`size-3.5 ${isEvaluating ? 'animate-spin' : 'fill-current'}`} />
          <span>{isEvaluating ? 'Đang tính toán...' : 'Chạy thử công thức'}</span>
        </button>
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-xl border border-border bg-muted/30 px-3.5 py-2 text-xs font-bold text-foreground hover:bg-muted transition-all shadow-xs disabled:opacity-50 cursor-pointer"
      ...
      {/* ── Right Action Buttons: Hint & Submit ── */}
      <div className="flex flex-wrap items-center gap-2">
        {/* Nút Gợi ý từng bước (Progressive Hints) */}
        <button
          type="button"
          onClick={onToggleHint}
          className="inline-flex items-center gap-2 rounded-xl border border-amber-500/30 bg-amber-500/10 px-3.5 py-2 text-xs font-bold text-amber-600 dark:text-amber-400 hover:bg-amber-500/20 transition-all shadow-xs cursor-pointer relative"
        ...
        <button
          ...
          onClick={onSubmit}
          disabled={isSubmitting}
          className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2 text-xs font-bold text-white hover:bg-emerald-700 transition-all shadow-md disabled:opacity-50 cursor-pointer"
