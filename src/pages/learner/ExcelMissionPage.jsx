...
import { formatDuration, difficultyLabel } from '../../utils/format.js';
...
export function ExcelMissionPage() {
  ...
  const starterCell = mission.starterContent?.targetCell || 'A1';
  ...
  return (
    <div className="flex flex-col min-h-[calc(100vh-5rem)] space-y-6 animate-fade-in pb-12">
      ...
      {/* ── Main Workspace Grid (Left Briefing Panel + Right Spreadsheet Container) ── */}
      <div className="grid lg:grid-cols-12 gap-6 items-start">
        ...
        {/* ── RIGHT PANEL: Spreadsheet Workspace Container (Shell Placeholder for Step 3.2) ── */}
        <div className="lg:col-span-8 space-y-4">
          <div className="rounded-3xl border border-border bg-card p-6 shadow-md min-h-[500px] flex flex-col justify-between relative overflow-hidden">
            {/* Top Workspace Header */}
            <div className="flex items-center justify-between border-b border-border pb-4 mb-4">
              <div className="flex items-center gap-3">
                <div className="grid size-9 place-items-center rounded-xl bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 font-mono font-bold text-sm">
                  FX
                </div>
                <div>
                  <h3 className="text-sm font-bold text-foreground">Spreadsheet Grid Workspace</h3>
                  <p className="text-xs text-muted-foreground">Không gian làm bài Excel interactive</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <span className="inline-flex items-center gap-1.5 rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-3 py-1 font-mono text-xs font-semibold text-emerald-600 dark:text-emerald-400">
                  <span className="size-2 rounded-full bg-emerald-500 animate-pulse" /> Ready
                </span>
              </div>
            </div>

            {/* Dataset Quick Preview Table */}
            {dataset ? (
              <div className="flex-1 space-y-3">
                <div className="flex items-center justify-between text-xs text-muted-foreground font-mono">
                  <span>Dữ liệu sơ bộ ({dataset.name}):</span>
                  <span>Target Cell: {starterCell}</span>
                </div>
                <div className="overflow-x-auto rounded-2xl border border-border bg-background">
                  <table className="w-full text-left text-xs border-collapse">
                    <thead>
                      <tr className="border-b border-border bg-muted/60 font-mono">
                        <th className="p-3 text-center w-12 text-muted-foreground font-bold border-r border-border">#</th>
                        {dataset.columns.map((col) => (
                          <th key={col.key} className="p-3 font-bold text-foreground border-r border-border min-w-[120px]">
                    ...
                    <tbody>
                      {dataset.rows.map((row, idx) => (
                        <tr key={idx} className="border-b border-border/60 hover:bg-muted/30 font-mono">
                          <td className="p-3 text-center text-muted-foreground font-bold border-r border-border bg-muted/20">
                            {idx + 2}
                          </td>
                          {dataset.columns.map((col) => {
                            const isTarget = col.excelColumn + (idx + 2) === starterCell;
                            return (
                              <td
                                key={col.key}
                                className={`p-3 border-r border-border/60 ${
                                  isTarget
                                    ? 'bg-amber-500/20 text-amber-600 dark:text-amber-400 font-bold border-2 border-amber-500/50'
                                    : 'text-foreground'
                                }`}
                              >
            ...
            <div className="mt-4 pt-3 border-t border-border flex items-center justify-between text-xs text-muted-foreground font-mono">
              <span>Hệ thống: Ready cho Step 3.2 Spreadsheet Grid</span>
              <span className="text-primary font-bold">Avi-Mystery Excel Engine v1.0</span>
            </div>
          </div>
        </div>
