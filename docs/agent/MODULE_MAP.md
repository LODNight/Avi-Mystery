...
|---|---|---|---|---|---|
| `LRN-EXCEL` | Excel Learning | Existing | `src/components/excel/`; `src/pages/learner/ExcelMissionPage*`; `src/utils/excelChecker*` | SHR, LRN-SUB | 1–3 |
| `LRN-SUB` | Submission & Feedback | Existing | `src/services/contracts/submissionService.js`; `src/services/index.js`; `src/services/mock/mockSubmissionService*`; submit flow trong `ExcelMissionPage*`; `MissionResultModal*`; `ActionToolbar*` | LRN-EXCEL, SHR | 3.4 |
...
| `SHR` | Shared Contracts/UI | Existing | `src/services/contracts/`; `src/services/index.js`; `src/components/ui/`; `src/hooks/`; `src/utils/`; `src/mocks/` | Không phụ thuộc feature module | Xuyên suốt |
